...
def generate_cover_letter(company_name, position_name, job_description, job_skills, resume_content):
    # Craft the prompt for the model to generate a cover letter
    prompt = f"Generate a customized cover letter using the company name: {company_name}, the position applied for: {position_name}"

    if job_description:
        prompt = prompt + f", and the job description: {job_description}."
    if job_skills:
        prompt = prompt + f", and the job skills keywords {job_skills}."
    
    prompt = prompt + f" Ensure the cover letter highlights my qualifications and experience as detailed in the resume content: {resume_content}. Adapt the content carefully to avoid including experiences not present in my resume but mentioned in the job description. The goal is to emphasize the alignment between my existing skills and the requirements of the role."
